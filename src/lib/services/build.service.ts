...
export class BuildService {
  ...
  github404HtmlContent(url: string, title = 'Sheetbase') {
    let subfolder = '';
    if (this.helperService.isHostSubfolder(url)) {
  ...
  async loadPrerendering(googleClient: OAuth2Client, databaseId: string) {
    ...
    if (await pathExists(prerenderConfigPath)) {
      ...
      // prerender items
      items = await this.parsePrerenderItems(
        googleClient,
        databaseId,
        rawItems
      );
  ...
  async parsePrerenderItems(
    ...
    for (let i = 0; i < (rawItems || []).length; i++) {
      ...
      if (typeof rawItem === 'string') {
        ...
        // load data
        const items = await this.spreadsheetService.getData(
          googleClient,
          databaseId,
          from
        );
  ...
  prerenderModifier(
    ...
    if (this.helperService.isHostSubfolder(url)) {
      html = this.helperService.replaceBetween(
        html,
        url + '/',
        '<base href="',
        '"'
    ...
    if (loadingScreen) {
      ...
      if (loadingScreen instanceof Object) {
        ...
        // default
        loadingHtml =
          '<div class="prerender-loading-screen"><div class="inner">LOADING...</div></div>';
        loadingCss =
          '.prerender-loading-screen{display:flex;position:fixed;height:100%;width:100%;left:0;top:0;background:#323639;text-align:center;justify-content:center;align-items:center}.prerender-loading-screen .inner {font-family:arial,sans-serif;color:#FFF}';
      }
      // replacing
      if (loadingHtml) {
